...
#   eixo/inicio/fim — por onde a frente de tinta varre, em espaço de mundo
#                     (projeção da posição no eixo, ver o shader)
...
const VARREDURAS: Array[Dictionary] = [
	{  # Leste (porta) — direita da garotinha. Varre do sul pro norte.
		"eixo": Vector3(0, 0, -1), "inicio": -2.0, "fim": 4.0,
		"de": Vector3(2.8, 0, 1.6), "para": Vector3(2.8, 0, -3.4), "angulo": -90.0,
	},
	{  # Norte (frente dela). Varre de leste pra oeste.
		"eixo": Vector3(-1, 0, 0), "inicio": -3.5, "fim": 3.5,
		"de": Vector3(2.8, 0, -3.4), "para": Vector3(-2.8, 0, -3.4), "angulo": 0.0,
	},
	{  # Oeste (janela) — esquerda dela. Varre do norte pro sul.
		"eixo": Vector3(0, 0, 1), "inicio": -4.0, "fim": 2.0,
		"de": Vector3(-2.8, 0, -3.4), "para": Vector3(-2.8, 0, 1.4), "angulo": 90.0,
	},
	{  # Sul (atrás dela) — fecha a volta. Varre de oeste pra leste.
		"eixo": Vector3(1, 0, 0), "inicio": -3.5, "fim": 3.5,
...
var _grupos_paredes: Array = []  # Array[Array[TintaSecando]]
...
func _retomar_de_save() -> void:
	...
	for i in range(_grupos_paredes.size()):
		_iniciar_demao_grupo(i, indice_cor)
		for no in _grupos_paredes[i]:
			var parede: TintaSecando = no
			parede.forcar_seco()
...
func _sequencia_abertura() -> void:
	...
	_iniciar_demao_grupo(PAREDE_ABERTURA, 0)
	_adiantar_grupo(PAREDE_ABERTURA, duracao_pintura_segundos * FRACAO_JA_PINTADA_ABERTURA)
...
func _rodada_de_pintura(indice_demao: int) -> void:
	...
	for i in range(_grupos_paredes.size()):
		...
		_iniciar_demao_grupo(i, indice_demao)
		await _tio.pintar_varrendo(de, para, duracao_pintura_segundos, angulo)

	await _tio_sai()

	estado = Estado.SECANDO
	await _esperar_secar(_grupos_paredes.size() - 1)


## Dispara a demão nas paredes de um grupo, com os dados de varredura dele.
func _iniciar_demao_grupo(indice_parede: int, indice_demao: int) -> void:
	var v: Dictionary   = VARREDURAS[indice_parede]
	var eixo: Vector3   = v["eixo"]
	var inicio: float   = v["inicio"]
	var fim: float      = v["fim"]
	var anterior: Color = _cor_antes_da_demao(indice_demao)
	for no in _grupos_paredes[indice_parede]:
		var parede: TintaSecando = no
		parede.iniciar_demao(
			anterior,
...
func _adiantar_grupo(indice_parede: int, segundos: float) -> void:
	for no in _grupos_paredes[indice_parede]:
		var parede: TintaSecando = no
		parede.adiantar(segundos)
...
func _esperar_secar(indice_ultima_pintada: int) -> void:
	var parede: TintaSecando = _grupos_paredes[indice_ultima_pintada][0]
	await parede.secagem_concluida


func _duracao_secagem_padrao() -> float:
	var primeira: TintaSecando = _grupos_paredes[0][0]
	return primeira.tempo_secagem_segundos
